...
bigram_ = pd.read_csv("data/trufeed.csv", index_col=0)
        ...
        html.A(
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        dbc.NavbarBrand("Trufeedback Data Analytics", className="ml-2")
                    ),
                ],
                align="center",
                no_gutters=True,
...
LEFT_COLUMN = dbc.Jumbotron(
    [
        html.H4(children="Select country & dataset size", className="display-5"),
        ...
        html.Label("Select country", style={"marginTop": 50}, className="lead"),
        html.P(
        ...
        [
            dcc.Loading(
                ...
                children=[
                    ...
                    dbc.Row(
                        [
                            dbc.Col(html.P("Choose 2 countries to compare:"), md=12),
                            dbc.Col(
                                [
                                    dcc.Dropdown(
                                        id="bigrams-comp_1",
...
def populate_bigram_scatter(perplexity):
    ...
    h = bigram_[bigram_["country"]==perplexity]['answers.3.values'].value_counts().values
    t = bigram_[bigram_["country"]==perplexity]['answers.3.values'].value_counts().index.values
...
def comp_bigram_comparisons(comp_first, comp_second):
    ...
    sizes = bigram_.loc[(comp_second)].groupby(['answers.2.values']).size().sort_values(ascending=False)[:10].values.shape[0]
    if sizes>0:
        sizeNew = 10+sizes
        compare.loc[10:sizeNew-1, 'ngram'] = bigram_.loc[(comp_second)].groupby(['answers.2.values']).size().sort_values(ascending=False)[:sizes].index
        compare.loc[10:sizeNew-1, 'value'] = bigram_.loc[(comp_second)].groupby(['answers.2.values']).size().sort_values(ascending=False)[:sizes].values

    #### first histo

    size = bigram_.loc[(comp_first)].groupby(['answers.2.values']).size().sort_values(ascending=False)[:10].values.shape[0]
    if size> 0:
        compare.loc[:size-1, 'ngram'] = bigram_.loc[(comp_first)].groupby(['answers.2.values']).size().sort_values(ascending=False)[:size].index
        compare.loc[:size-1, 'value'] = bigram_.loc[(comp_first)].groupby(['answers.2.values']).size().sort_values(ascending=False)[:size].values
